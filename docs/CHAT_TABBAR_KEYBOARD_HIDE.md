...
При открытии клавиатуры в экранах чата (DirectChatScreen, GroupChatScreen) и поиска (SearchScreen), нижний CustomTabBar оставался видимым, занимая ценное пространство экрана и мешая просмотру контента и полю ввода.
...
Реализовано автоматическое скрытие/показ нижнего таб-бара при открытии/закрытии клавиатуры **в экранах чата и поиска** с использованием Context API для управления видимостью на уровне навигатора.
...
#### 4. Проверка экрана чата и поиска:
Реализована многоуровневая проверка для определения, находится ли пользователь в чате или поиске:
...
### Почему только в чате и поиске?

Скрытие TabBar происходит **только в экранах чата и поиска**, потому что:
1. В чате клавиатура открывается часто и надолго, нужно максимум места для просмотра сообщений
2. В поиске активно используется поле ввода, нужно больше места для результатов
3. В других экранах (главная, корзина, профиль) TabBar не мешает работе
...
3. **Закрытие клавиатуры:**
   - [ ] TabBar появляется обратно
   - [ ] Нет задержек или глюков

4. **Переключение между табами:**
   - [ ] В других табах (Главная, Корзина, Профиль) TabBar не скрывается
   - [ ] TabBar остается видимым при вводе в других формах

5. **Различные типы чатов:**
...
6. **Переход между экранами:**
   ...
   - [ ] Обратно из чата/поиска - TabBar появляется
