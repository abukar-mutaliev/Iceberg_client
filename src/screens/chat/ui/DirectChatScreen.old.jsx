...
export const DirectChatScreen = ({route, navigation}) => {
    const {
        ...
        autoSendProduct,
        userId,
        autoFocusInput = false, // Автоматический фокус на поле ввода для быстрого ответа
    } = route.params || {};
    ...
    // Стабильный userId собеседника для зависимостей
    // Используем только примитивные значения для стабильности
    ...
    useEffect(() => {
        // Сбрасываем флаг при монтировании или изменении roomId
        if (roomId) {
            isRoomDeletedRef.current = false;
        }
        
        // Проверяем удаление только если roomData уже загружен или комната помечена как удаленная
        // При холодном запуске roomData может быть undefined, но это не означает, что комната удалена
        if (isRoomDeleted && roomId) {
            ...
            // Дополнительная проверка: если roomData загружен и помечен как удаленный
            isRoomDeletedRef.current = true;
            dispatch(setActiveRoom(null));
            emitActiveRoom?.(null);
    ...
    // Активная комната и уведомления - устанавливаем СРАЗУ через useLayoutEffect
    // Это критично для подавления уведомлений, которые могут прийти до рендера
    useLayoutEffect(() => {
        if (!roomId) return;
        
        // Устанавливаем активную комнату СРАЗУ, даже если userId еще не определен
        // Это важно для подавления уведомлений, которые могут прийти до загрузки данных
        ...
        if (__DEV__) {
            console.log('[DirectChatScreen] 🔄 [useLayoutEffect] Устанавливаем активную комнату СРАЗУ', { 
                roomId,
                userId: peerUserId,
        ...
        return () => {
            // Cleanup ТОЛЬКО при размонтировании компонента (когда roomId меняется на другой или компонент размонтируется)
            if (__DEV__) {
                console.log('[DirectChatScreen] 🗑️ [useLayoutEffect] Cleanup: сбрасываем активную комнату', { roomId });
            }
            PushNotificationService.setActiveChatRoomId(null);
            PushNotificationService.setActiveChatPeerUserId(null);
        };
    }, [roomId, userId]); // roomId и userId в зависимостях - обновляем только при их изменении
    
    // Активная комната и уведомления - полная инициализация через useEffect
    // Используем стабильный peerUserIdStable вместо chatPartner в зависимостях
    useEffect(() => {
        if (isRoomDeletedRef.current || isRoomDeleted || !roomId) {
            if (__DEV__) {
                console.log('[DirectChatScreen] ⚠️ Пропуск установки активной комнаты', {
        ...
        // Используем стабильный peerUserIdStable
        const peerUserId = peerUserIdStable;
        
        // Обновляем активную комнату (на случай, если userId изменился)
        // Но НЕ сбрасываем в null - это уже установлено в useLayoutEffect
        // Просто обновляем userId, если он изменился
        ...
        const unsubscribe = navigation.addListener('focus', () => {
            if (!isRoomDeletedRef.current && !isRoomDeleted) {
                // Обновляем активную комнату при фокусе (на случай, если userId изменился)
    ...
    const handleRetryMessage = useCallback(async (message) => {
        ...
        try {
            if (message.type === 'VOICE') {
                ...
                if (!message.content) {
                    throw new Error('Содержимое сообщения не найдено');
                }
                ...
                if (!message.poll && !message.content) {
                    throw new Error('Данные опроса не найдены');
                }
